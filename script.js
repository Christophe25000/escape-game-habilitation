...
function loadTableau3() {
    ...
    document.getElementById("game-container").innerHTML = `
        ...
        <button onclick="validateAssociations()" style="margin-top:20px;">Valider</button>
    `;
...
function initDragDrop() {
    const imgs = document.querySelectorAll(".t3-img");
    const zones = document.querySelectorAll(".t3-dropzone");
    ...
    zones.forEach(zone => {
        ...
        zone.addEventListener("drop", e => {
            ...
            zone.dataset.current = id;

            zone.style.background = "#e0f0ff";
...
function validateAssociations() {
    ...
    zones.forEach(zone => {
        if (zone.dataset.current !== zone.dataset.expected) {
            zone.style.background = "#ffdddd";
            zone.style.borderColor = "red";
            errors++;
        } else {
            zone.style.background = "#ddffdd";
            zone.style.borderColor = "green";
        }
    });

    if (errors > 0) {
        alert("❌ Certaines associations sont incorrectes.");
        return;
    }
...
function loadTableau4() {
    ...
    ">
        ...
        ">
            ...
            ">
                ${shuffled.map(step => `
                    <div class="t4-card" draggable="true" data-step="${step}"
                         style="
                            padding:12px;
 ...
 * TABLEAU 4 – DRAG & DROP
 ****************************************************/
function initDragDropT4() {
    const cards = document.querySelectorAll(".t4-card");
    ...
    slots.forEach(slot => {
        ...
        slot.addEventListener("drop", e => {
            ...
            slot.innerHTML = `
                <div class="t4-card" data-step="${step}"
                     style="padding:10px; border:2px solid #003366; background:white;">
                    ${step}
                </div>
            `;
        });
...
function validateTableau4() {
    const slots = document.querySelectorAll(".t4-slot");
    const userOrder = Array.from(slots).map(s => s.textContent.trim());

    for (let i = 0; i < tableau4Steps.length; i++) {
        if (userOrder[i] !== tableau4Steps[i]) {
    ...
    // 🔥 ENVOI GOOGLE SHEETS ICI
    fetch("https://script.google.com/macros/s/AKfycbwlQVjPAc-hSxNb_2USAmSXCIT888VdEVp8QqSQy2WYdgY9F325d9nJpZCpp0XWYJudDQ/exec", {
        method: "POST",
        mode: "no-cors",
        headers: {
        ...
        body: JSON.stringify({
            ...
            success: "OK",
            session: window.currentSessionCode   // ← ton code de session
        })
    });

    // 🔥 Popup final
    const popup = document.createElement("div");
    popup.id = "popup-end-final";
    popup.className = "popup";

    popup.innerHTML = `
        <div class="popup-content">
            <h2>🎉 Bravo ${playerName} !</h2>
            <p>Vous avez terminé les 4 tableaux.</p>
